...
Rcpp::IntegerVector classify(std::vector<double> v, std::vector<bool> e, int samplerate, double vt, double sigma, double minblink, double minsac, double glswin);

double sigthresh(std::vector<double> x, std::vector<bool> e, double threshold, double sigma);

std::vector<int> uidvec(std::vector<bool> x, int start);
std::vector<int> ruidvec(std::vector<std::string> x, int start);
